...
document.title = "Brands List"

import { ref, onMounted } from 'vue';
import { useRouter } from 'vue-router';

import { AuthStore } from '@/stores/auth';
import { listCreateManufacturerUrl } from '@/endpoints';
...
onMounted(() => {
  getData(listCreateManufacturerUrl)
})
...
<template>
    ...
    <template v-else >
      <div class="row mb-3">
        <Heading :title="'Manufactuer Brands (' + count + ')'" />  
        <div class="col-md-12 col-lg-12">
            <ul v-if="count > 0" class="list-group list-group-flush">
                <li v-for="manufactuer in results" :key="manufactuer.id" class="list-group-item ps-0">
                    {{ manufactuer.name }}
                </li>
            </ul>
            <h2 v-else>No manufactuer found</h2>
        </div>
      </div>
    </template>
      </div>
